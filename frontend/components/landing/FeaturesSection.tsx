    ...
    <div className={`relative z-10 bg-slack-surface-primary px-4 sm:px-6 ${className}`.trim()}>
      <section id="readiness" className="mx-auto w-full max-w-6xl py-14 sm:py-20">
        <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slack-primary">
              Discover your readiness
            </p>
            <h2 className="mt-4 text-2xl font-semibold text-slack-text-primary sm:text-3xl lg:text-4xl">
              Discover your startup readiness
            </h2>
            <p className="mt-4 max-w-xl text-sm text-slack-text-secondary sm:text-base">
            ...
            <div className="mt-6 grid gap-3 sm:mt-8 sm:grid-cols-2 sm:gap-4">
              {READINESS_STEPS.map((step) => (
                <div
                  key={step}
                  className="rounded-2xl border border-slack-border bg-slack-surface-secondary px-4 py-4 text-sm text-slack-text-secondary"
                >
                  {step}
                </div>
              ))}
          ...
          <div className="rounded-3xl border border-slack-border bg-slack-surface-primary p-6 shadow-slack-lg sm:p-8">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slack-text-secondary">
                  Let&apos;s get to know you
                </p>
                <p className="mt-2 text-lg font-semibold text-slack-text-primary">Readiness checkpoint</p>
              </div>
            ...
            <div className="mt-5 space-y-3 sm:mt-6">
              {[
                ...
                <div
                  key={item}
                  className="flex items-center gap-3 rounded-xl border border-slack-border bg-slack-surface-secondary px-4 py-3 text-sm text-slack-text-secondary"
                >
                  <span className="flex h-6 w-6 items-center justify-center rounded-full border border-slack-border text-[11px] text-slack-text-secondary">
                    {index + 1}
                  </span>
                  <span>{item}</span>
                </div>
            ...
            <div className="mt-5 h-2 w-full overflow-hidden rounded-full bg-slack-surface-tertiary sm:mt-6">
              <div className="h-full w-2/3 rounded-full bg-slack-primary" />
            </div>
            <div className="mt-5 flex items-center justify-between text-xs uppercase tracking-[0.2em] text-slack-text-secondary sm:mt-6">
              <span>Progress</span>
              <span>66%</span>
            </div>
            <button
              type="button"
              className="mt-6 w-full rounded-pill bg-slack-surface-tertiary py-3 text-xs font-semibold uppercase tracking-[0.25em] text-slack-text-primary transition-colors hover:bg-slack-border"
            >
              Continue
            </button>
          </div>
      ...
      <section id="system" className="mx-auto w-full max-w-6xl py-14 sm:py-20">
        <div className="rounded-3xl border border-slack-border bg-slack-surface-secondary p-6 sm:p-10">
          <div className="relative grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <p className="mt-4 text-sm text-slack-text-secondary sm:text-base">
              ...
              <Link
                href="#integration"
                className="mt-6 inline-flex rounded-pill border border-slack-border px-6 py-3 text-xs font-semibold uppercase tracking-[0.25em] text-slack-text-secondary transition-colors hover:bg-slack-surface-tertiary sm:mt-8"
              >
                Explore more
              </Link>
            </div>
            <FeatureGrid className="grid gap-4">
              {SYSTEM_MODULES.map((module) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-lg font-semibold text-slack-text-primary">{module.title}</p>
                      <p className="text-xs uppercase tracking-[0.2em] text-slack-text-secondary">
                        {module.role}
                      </p>
                    </div>
                    <span className="rounded-pill border border-slack-border bg-slack-surface-tertiary px-3 py-1 text-[10px] uppercase tracking-[0.25em] text-slack-text-secondary">
                      Active
                    </span>
                  </div>
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {module.stats.map((stat) => (
                      <div
                        key={stat.label}
                        className="rounded-xl bg-slack-surface-secondary px-3 py-2"
                      >
                        <p className="text-sm font-semibold text-slack-text-primary sm:text-base">
                          {stat.value}
                        </p>
                        <p className="mt-1 text-[9px] uppercase tracking-[0.16em] text-slack-text-secondary leading-snug break-words sm:text-[10px] sm:tracking-[0.2em]">
                          {stat.label}
                        </p>
                      </div>
                    ))}
      ...
      <section id="integration" className="mx-auto w-full max-w-6xl py-14 sm:py-20">
        <div className="text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-sm text-slack-text-secondary sm:text-base">
      ...
      <section id="community" className="mx-auto w-full max-w-6xl py-14 sm:py-20">
        <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <div className="mt-5 space-y-3 text-sm text-slack-text-secondary sm:mt-6">
            ...
            <div className="mt-6 rounded-2xl border border-slack-border bg-slack-surface-secondary p-5 sm:mt-8 sm:p-6">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slack-text-secondary">
                Trusted by
              </p>
              <div className="mt-4 flex flex-wrap items-center gap-3">
                {TRUSTED_PARTNERS.map((partner) => (
                  <span
                    key={partner}
                    className="rounded-pill border border-slack-border bg-slack-surface-tertiary px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-slack-text-secondary"
                  >
                    {partner}
                  </span>
                ))}
          ...
          <div className="relative overflow-hidden rounded-3xl border border-slack-border bg-slack-surface-secondary aspect-[4/3] sm:aspect-[16/10]">
            <Image
              src="/image.png"
            ...
            <div
              aria-hidden
              className="absolute inset-0 bg-gradient-to-t from-slack-surface-secondary/70 via-transparent to-transparent"
            />
            <div className="absolute bottom-4 left-4 right-4 rounded-2xl border border-slack-border bg-slack-surface-secondary/70 px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.2em] text-slack-text-primary backdrop-blur">
      ...
      <section id="stories" className="mx-auto w-full max-w-6xl py-14 sm:py-20">
        <div className="flex flex-wrap items-center justify-between gap-6">
          ...
          <Link
            href="/auth/register"
            className="rounded-pill border border-slack-border px-6 py-3 text-xs font-semibold uppercase tracking-[0.25em] text-slack-text-secondary transition-colors hover:bg-slack-surface-tertiary"
          >
            Join them
          </Link>
        </div>
      ...
      <section id="contact" className="mx-auto w-full max-w-6xl pb-20 pt-8 sm:pb-28 sm:pt-10">
        <div className="rounded-3xl border border-slack-border bg-slack-surface-secondary p-6 text-center sm:p-10">
          <div className="relative">
            ...
            <div className="mt-6 flex flex-col items-stretch justify-center gap-3 sm:mt-8 sm:flex-row sm:items-center sm:gap-4">
              <Link
                href="/auth/register"
                className="w-full rounded-pill bg-slack-primary px-7 py-3 text-center text-xs font-semibold uppercase tracking-[0.25em] text-slack-text-inverse shadow-slack-lg transition-transform hover:-translate-y-0.5 sm:w-auto"
              >
                Start Journey
              </Link>
              <a
                href="mailto:hello@chatterchat.io"
                className="w-full rounded-pill border border-slack-border px-7 py-3 text-center text-xs font-semibold uppercase tracking-[0.25em] text-slack-text-secondary transition-colors hover:bg-slack-surface-tertiary sm:w-auto"
              >
                Talk to us
              </a>
            </div>
